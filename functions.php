...
if (function_exists('add_theme_support')) {
    ...
    add_image_size('mobile', 768, '', true); // Custom Thumbnail Size call using the_post_thumbnail('mobile');
    add_image_size('tablet', 1280, '', true); // Custom Thumbnail Size call using the_post_thumbnail('tablet');
    add_image_size('laptop', 1640, '', true); // Custom Thumbnail Size call using the_post_thumbnail('laptop');
    add_image_size('desktop', 1920, '', true); // Custom Thumbnail Size call using the_post_thumbnail('desktop');
    add_image_size('services', 464, 309, true);
    add_image_size('services-card', 720, 432, true);
    add_image_size('services-large', 1400, 700, true);
    ...
    function change_logo_class( $html ) {
        ...
        $html = str_replace( 'custom-logo-link', 'w-auto max-h-full flex flex-row items-center', $html );
        $html = str_replace( 'custom-logo', 'w-auto', $html );
...
function footer_nav_left()
{
	...
	array(
		...
		'link_before'     => '',
		'link_after'      => '',
		'items_wrap'      => '<ul class="text-brand-clay uppercase text-sm">%3$s</ul>',
		'depth'           => 0,
        'add_li_class'    => '',
		'walker'          => false
		)
...
function footer_scripts()
{
    ...
    //init all swipers
    wp_register_script('homepage-swiper', get_template_directory_uri() . '/js/swiper/swiper-inits.js');
    wp_enqueue_script('homepage-swiper'); // Enqueue it!

    //init anchor links on about page 
    wp_register_script('anchor-links', get_template_directory_uri() . '/js/anchor-links.js');
    wp_enqueue_script('anchor-links'); // Enqueue it!
...
function register_menu()
{
    register_nav_menus(array( // Using array to specify more menus if needed
        ...
        'footer-menu-left' => __('Footer Left', 'web-ok-starter'), // Footer Left Navigation
        'footer-menu-right' => __('Footer Right', 'web-ok-starter'), // Footer Right Navigation
    ));
}
